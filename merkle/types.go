...
type MKRoot interface {
	Eq(typeclass.Eq) bool
	NotEq(typeclass.Eq) bool
...
// functional Eq
func (a *MerkleRoot) Eq(b typeclass.Eq) bool {
	v, ok := b.(*MerkleRoot)
	if !ok {
		return false
	}
	return bytes.Compare(a.getMerkleRoot(), v.getMerkleRoot()) == 0
}

// functional Eq
func (a *MerkleRoot) NotEq(b typeclass.Eq) bool {
	return !a.Eq(b)
}
